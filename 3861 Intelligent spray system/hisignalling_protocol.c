...
/*Define smart spray system pins*/
//Pump 1
...
/*Intelligent spray system initialization*/
void Spraysystem_init(void)
{
    // Initialize GPIO10
    IoTGpioInit(IOT_GPIO_IDX_10);
    // Set GPIO10 as the output direction
    IoTGpioSetDir(IOT_GPIO_IDX_10, IOT_GPIO_DIR_OUT);
    // Initialize GPIO9
    IoTGpioInit(IOT_GPIO_IDX_9);
    // Set GPIO9 as the output direction
    IoTGpioSetDir(IOT_GPIO_IDX_9, IOT_GPIO_DIR_OUT);
}

/**
* @brief:Turn on the smart spray system
The data packet sent by 3516 was analyzed to obtain the information of the number, 
species and distance of pests, and the control of spraying dose, spraying type 
and spraying time was realized. 
respectively.
* @param a: Spraying time
* @param b: Spray cooling time
Analog PWM technology is used to control the dosage of spray. 
The greater the a value, the greater the spray dose.
* @param c: Spray classification
If c==1, identify as a caterpillar and turn on pump 1. 
If c==2, identify as other and turn on pump 2.
* @param d: Spray distance
If d==1, it indicates that the spraying distance has been reached 
and the intelligent spraying system can be activated.
**/
void Spray_on(int a,int b,int c,int d)
{
    if (d==1) {
        if (c==1) { 
...
/*Turn off the smart spray system*/
void Spray_off(void)
{   
    /*Turn off pump 1*/
    IoTGpioSetOutputVal(IOT_GPIO_IDX_10, IOT_GPIO_VALUE0);
...
hi_u32 HisignalingMsgTask(hi_void)
{
    ...
    if (osThreadNew((osThreadFunc_t)HisignallingMsgHandle, NULL, &hisignallingAttr) == NULL) {
        printf("Failed to create hisignaling msg task\r\n");
        return HI_ERR_FAILURE;
    }
    return HI_ERR_SUCCESS;
}
